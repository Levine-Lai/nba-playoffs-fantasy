...
- `CourtPlayerCard` on both `/edit-lineup` and `/points` now keeps the headshot layer at roughly `99%` card width, lets the head extend above the card top, uses the colored top panel as the visible portrait mask, keeps the lower body flush with the mask bottom, and adds only a very light bottom shadow gradient for depth; the name and score sections sit above the portrait so the upper image stays exposed while the lower body is neatly covered by the card content. / `/edit-lineup` 和 `/points` 的 `CourtPlayerCard` 现在保持头像浮层大约为卡片宽度的 `99%`，让头部可以高出卡片顶部，用顶部彩色区域作为人物可见的遮罩窗口，并让人物下半身紧贴遮罩底边不留空白，同时只加入一层很轻的底部暗影渐变来增强立体感；名字和分数区域则压在人物上方，从而形成上半部分外露、下半部分被卡片内容自然遮住的效果。
