...
- `CourtPlayerCard` on both `/edit-lineup` and `/points` now uses a portrait-first layout with a floating headshot layer sized to roughly `99%` of the card width and positioned above the card top so the head visibly rises past the upper edge; `/points` also switches to a pink-purple gradient highlight when `points / salary > 5` or `points > 50`. / `/edit-lineup` 和 `/points` 的 `CourtPlayerCard` 现在使用以人物为主的版式，头像浮层宽度大约是卡片宽度的 `99%`，并且会高出卡片顶部，让头部明显超过上边缘；同时 `/points` 在 `points / salary > 5` 或 `points > 50` 时会切换为粉紫渐变高亮。
