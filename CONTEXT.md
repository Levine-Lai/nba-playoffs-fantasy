...
- `CourtPlayerCard` on both `/edit-lineup` and `/points` now keeps the headshot layer at roughly `99%` card width, anchors the PNG bottom edge to the divider line, aims for an overall portrait-card proportion close to `3:5` measured from the top of the headshot to the card bottom, uses the colored top panel as the visible portrait mask, and adds only a very light bottom shadow gradient for depth; the name and score sections still sit above the portrait so the upper image stays exposed while the lower body is neatly covered by the card content. / `/edit-lineup` 和 `/points` 的 `CourtPlayerCard` 现在保持头像浮层大约为卡片宽度的 `99%`，并把 PNG 底边锚定到分隔线位置，同时让从头像最上沿到卡片底部的整体比例尽量接近 `3:5`；顶部彩色区域继续作为人物可见的遮罩窗口，并只加入一层很轻的底部暗影渐变来增强立体感；名字和分数区域仍然压在人物上方，从而形成上半部分外露、下半部分被卡片内容自然遮住的效果。
