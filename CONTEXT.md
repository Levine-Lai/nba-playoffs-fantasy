...
- `CourtPlayerCard` on both `/edit-lineup` and `/points` now lowers the visible card body itself when a portrait exists, so the rounded card top sits farther down while the player head clearly protrudes outside the card; the PNG bottom edge still anchors to the divider line, and the top colored panel remains the portrait mask with only a very light bottom shadow gradient. / `/edit-lineup` 和 `/points` 的 `CourtPlayerCard` 现在会在有人像头像时把可见卡片本体整体下移，让圆角卡片顶部明显更靠下，从而让球员头部清楚地露出卡片外；同时 PNG 底边仍然锚定在分隔线上，顶部彩色区域继续作为人物遮罩，并只保留一层很轻的底部暗影渐变。
