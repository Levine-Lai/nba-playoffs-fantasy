...
- `/api/standings` refreshes current-period points before ranking, and the Standing page polls while visible so ordering stays aligned with Points. / `/api/standings` 会在排名前刷新当前计分日分数，Standing 页面在可见时轮询刷新，以保持与 Points 一致。
