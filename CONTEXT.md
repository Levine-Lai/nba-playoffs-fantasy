...
- `CourtPlayerCard` on both `/edit-lineup` and `/points` now keeps the headshot layer at roughly `99%` card width, anchors the PNG bottom edge to the divider line, lets the head extend slightly above the card top, and reclaims space by tightening the internal name band plus the bottom schedule or score area; the visible portrait still uses the colored top panel as a mask with only a very light bottom shadow gradient for depth. / `/edit-lineup` 和 `/points` 的 `CourtPlayerCard` 现在保持头像浮层大约为卡片宽度的 `99%`，并把 PNG 底边锚定到分隔线位置，让头部略微伸出卡片顶部；同时通过收紧名字区和底部赛程或分数区来腾出空间。人物可见区域仍然使用顶部彩色面板作为遮罩，并只保留一层很轻的底部暗影渐变来增强立体感。
