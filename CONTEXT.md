...
- `CourtPlayerCard` on both `/edit-lineup` and `/points` now lowers the visible card body itself when a starter portrait exists, so the rounded card top sits farther down while the player head clearly protrudes outside the card; compact bench cards no longer request or render player headshots, leaving only the colored top panel to reduce image loading. / `/edit-lineup` 和 `/points` 的 `CourtPlayerCard` 现在会在首发区存在人物头像时把可见卡片本体整体下移，让圆角卡片顶部明显更靠下，从而让球员头部清楚地露出卡片外；而紧凑的替补席卡片则不再请求或渲染球员头像，只保留顶部彩色面板，以减少图片加载。
