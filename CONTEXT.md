...
- `Points` now supports a page-scoped pixel basketball court background on the main content column only, using the generated static asset at `frontend/public/backgrounds/points-pixel-court.png`; the court sits under a soft translucent shell so the lineup cards and panels remain readable while the sidebar keeps its original plain treatment. Compact bench cards also use a much shorter top color panel after headshots were removed, so the old empty portrait space is cropped away instead of remaining as blank padding. / `Points` 页面现在支持只作用于主内容列的像素风篮球场背景，使用生成后的静态资源 `frontend/public/backgrounds/points-pixel-court.png`；球场背景会放在一个柔和的半透明外壳之下，让阵容卡片和面板仍然清晰可读，而右侧边栏保持原本的纯净样式。与此同时，替补席紧凑卡在去掉头像后也会使用更短的顶部彩色区，把原先为头像保留的空白直接裁掉，不再留下空顶。
