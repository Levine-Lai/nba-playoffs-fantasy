...
- `Points` now loads the generated pixel court image only inside the `Starting 5` formation area, with the portrait-oriented court scaled and centered to fit the two-row layout instead of sitting behind the whole page column. Compact bench cards keep the cropped top color strip after headshots were removed, and their name blocks are now larger so the card body feels fuller and more balanced. / `Points` 页面现在只会在 `Starting 5` 阵型区域内部加载生成的像素球场图，并把这张竖版球场图按两排首发卡的布局做居中和比例适配，而不再铺在整列页面内容下面。替补席紧凑卡在去掉头像后仍保留裁短后的顶部彩色条，同时名字区也进一步放大，让整张卡片更饱满、更舒适。
