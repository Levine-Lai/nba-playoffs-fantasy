...
- The `/schedule` page is currently back on the stable version: the original grouped gameday list remains the main view, and the April calendar preview stays at the bottom using one matchup row per game with away on the left, home on the right, a `Day1`-style badge in the date header, and an `R1G1`-style label beneath the score. / `/schedule` 页面当前已回到稳定版本：原来的按比赛日分组列表继续作为主视图，底部保留 4 月日历预览；日历里每场比赛按单独一行展示，左侧客队、右侧主队，日期头右侧显示 `Day1` 这种样式的标记，比分下方显示 `R1G1` 这种样式的标签。
...
- The `/standings` API now refreshes each roster's current scoring-period points before ranking, so live totals and ordering stay aligned with the `Points` page instead of depending on whoever most recently opened their own profile. / `/standings` API 现在会在排名前先刷新各队当前计分周期的分数，因此实时总分和排序会与 `Points` 页面保持一致，而不再依赖谁最近刚好打开过自己的资料页。
- The `Standing` page also polls the refreshed standings payload while visible, so live leaderboard updates no longer require a manual page reload. / `Standing` 页面在可见状态下也会轮询刷新后的 standings 数据，因此实时榜单更新不再需要手动刷新页面。
